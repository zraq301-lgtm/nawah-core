...
// استيراد الإعدادات (تأكد من صحة المسار بناءً على مكان الملف الحالي)
...
const Dashboard = ({ stats }) => {
  ...
  // قائمة الأقسام مع المسارات المطابقة للموجود في App.jsx
  const sections = [
  ...
  return (
    <div className="p-4 md:p-6 text-right animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans" dir="rtl">
      ...
      {/* 1. الترويسة العلوية */}
      <div className="bg-gradient-to-br from-slate-800 to-slate-700 p-6 rounded-[24px] text-white mb-6 relative overflow-hidden shadow-xl">
        <div className="relative z-10">
          <h1 className="text-2xl font-black mb-1">{clientName}</h1>
          <p className="opacity-70 text-sm">لوحة الإدارة الذكية - الإصدار السحابي</p>
        </div>
        <Settings size={120} className="absolute -left-6 -bottom-6 opacity-5 text-white animate-spin-slow" />
      </div>

      {/* 2. الإحصائيات المالية (Stats) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={i} className="bg-white/50 backdrop-blur-lg border border-white/60 shadow-md p-4 rounded-3xl text-center flex flex-col items-center justify-center transition-transform hover:scale-105 hover:bg-white/70">
      ...
      {/* 3. شبكة الأقسام (Modules) */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {sections.map((sec) => (
          <div
            key={sec.id}
            onClick={() => navigate(sec.path)}
            className={`bg-white/40 backdrop-blur-md border-t-4 ${sec.border} border-x border-b border-white/60 shadow-sm p-5 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-white/80 active:scale-95 transition-all duration-300 group`}
          >
      ...
      <div 
        ...
        className="mt-8 bg-white/40 backdrop-blur-md border border-white/60 p-4 rounded-2xl flex items-center justify-center gap-2 cursor-pointer text-slate-500 font-bold hover:bg-white hover:text-pink-500 transition-all shadow-sm"
      >
